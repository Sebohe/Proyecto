...
void SourceLocalization::init(){
	...
	try{
		...
		audio.setParameter("Sensibility", 0.8);
...
/*Lo que hace el programa es emter en el subscribe to event... y se queda adentro de el hasta que hacemos que temrmine el programa.si queremos que haga algo antes de empezar el subscibe podemos ver el ejemplo de onfacedetection donde le pide a ver si antes de inciar por lo menos hay algun rostro.
*/
...
void SourceLocalization::soundLocated(){
	...
	if(((azimuth==0.0f)&&(elevacion==0.0f))||(confidence<0.7f)){
		...
		*/
		...
		//motion.angleInterpolation(jointName, targetAnglesCero, targetTimesCero, isAbsolute);
	}// if de confidence
}//SourceLocalization
