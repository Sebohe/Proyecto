  ...
  const std::string& name) : AL::ALModule(broker, name)
{
  ...
  fCommand = "speak";
  ALSpeechBasedReaction::startRecognition();

}

ALSpeechBasedReaction::~ALSpeechBasedReaction()
{
  ALSpeechBasedReaction::stopRecognition();
}
...
void ALSpeechBasedReaction::onSpeechRecognized(const std::string& name,const AL::ALValue& val,const std::string& myName){
  ...
  for(unsigned int i = 0; i < val.getSize()/2 ; ++i)
  {
    ...
    {
      fTextToSpeech->callVoid("say",std::string("ok, Good Night!"));
	  ALSpeechBasedReaction::stopRecognition();
    }
  }
}
